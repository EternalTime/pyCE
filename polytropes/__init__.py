...
class polytrope:
    ...
    ----------------------------------------------------------------------------
    """
    def __init__(self,n = 1.5,dr = .001):
        self.n = n
        self.gamma = 1+1.0/n
        self.dr = dr
    ...
    def __solve_Lane_Emden__(self):
        ...
        F2 = lambda r,theta,psi: -2*psi/r - theta**self.n
        theta  = [1.0, 1.0]
        psi = [-np.finfo(np.float128).eps,-np.finfo(np.float128).eps]
        ...
        while theta[-1] > 10**-5:
			try:
				...
				theta.append(theta[-1] + self.dr/6.*(k31+2.*k32+2.*k33+k34))
				psi.append(psi[-1] + self.dr/6.*(k41+2.*k42+2.*k43+k44))
