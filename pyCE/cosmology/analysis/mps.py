r"""Information-theoretic analysis of matter power spectra.
...
continuous wavenumber grid, with the d-dimensional Jacobian factor
:math:`x^{d-1}` included in the measure. All integrals use the trapezoidal
rule.
"""
...
def norm(x,y,d = 3):
    r"""Normalize y over x with the d-dimensional radial measure.
    ...
    -------
    ndarray
        y divided by :math:`\int y \, x^{d-1} \, dx`.
    """
    return y/np.trapezoid(y*x**(d-1),x)

def modal_fraction(x,y,d = 3,xmax = np.inf,xmin = 0):
    r"""Modal fraction of a power spectrum over a restricted k-range.
    ...
    factor :math:`x^{d-1}` so the result is a probability density in x.
...
def KL_divergence(x,p,q,xmin = 0,xmax = np.inf):
    ...
    -------
    float
        :math:`\int p \, \log_2(p/q) \, dx` over the valid range.
    """
    #finds the valid k-modes
...
def entropy(x,p):
    r"""Differential Shannon entropy of a modal fraction, in bits.
